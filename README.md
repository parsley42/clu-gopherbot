...
### Run From Robot Home (Important)

Run `gopherbot` from the robot home directory (for example `clu/`), not from `clu/custom/`.

`custom/` is the cloned robot configuration repository, but `.env` (including `GOPHER_ENCRYPTION_KEY`) lives in the robot home. Running from `custom/` can fail startup/validation because the key is not loaded.

Examples (from `clu/`):
```shell
./gopherbot
../gopherbot/gopherbot validate custom
../gopherbot/gopherbot dump configured custom/conf/robot.yaml
```
