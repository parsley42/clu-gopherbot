...
if cmd == "lua" then
    ...
    local retVal6 = bot:MessageFormat(fmt.Fixed)
    if retVal6 == ret.Ok then
        bot:Log(log.Info, "Test 6 Passed: MessageFormat set to Fixed.")
    else
        bot:Log(log.Error, "Test 6 Failed: MessageFormat returned an error.")
    end
    bot:Log(log.Info, "Test 6: Current Message Format: " .. tostring(bot.format))
    bot:Say("Hello with Fixed formatting!")
    ...
    local retVal7 = bot:MessageFormat(fmt.Variable)
    if retVal7 == ret.Ok then
        bot:Log(log.Info, "Test 7 Passed: MessageFormat set to Variable.")
    else
        bot:Log(log.Error, "Test 7 Failed: MessageFormat returned an error.")
    end
    bot:Log(log.Info, "Test 7: Current Message Format: " .. tostring(bot.format))
    bot:Say("Hello with Variable formatting!")
    ...
    if retVal15 == ret.Ok then
        ...
        bot:Log(log.Info, "Test 15 Passed: Reply to Jill returned an error as expected.")
    end
